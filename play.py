...
class Quadratic:
    ...
    def d_dt_dsq_at_t_v2(self, t, p):
        ...
        a = (p0*p0 -2*p1*p0+p2*p0)*t3 + (-3*p0*p0+5*p1*p0-2*p2*p0)*t2 + (3*p0*p0-4*p1*p0+1*p2*p0)*t + (-1*p0*p0+1*p0*p1)
        a_orig = -u2*u*self.pts[0]*self.pts[0] + u_m_t*u2*self.pts[1]*self.pts[0] + t*u2*self.pts[2]*self.pts[0]
        # a == a_orig
        ...
        b = (-p0*p1 + 2*p1*p1 - p2*p1)*t3 + (2*p0*p1-3*p1*p1+1*p2*p1)*t2 + (-1*p0*p1+1*p1*p1)*t
        b_orig = -u*ut*self.pts[0]*self.pts[1] + u_m_t*ut*self.pts[1]*self.pts[1] + t*ut*self.pts[2]*self.pts[1]
        # b == b_orig
        ...
        c_orig = -u*t2*self.pts[0]*self.pts[2] + u_m_t*t2*self.pts[1]*self.pts[2] + t3*self.pts[2]*self.pts[2]
        # c == c_orig
        # p * p'(t) / 2
        # d = -u*self.pts[0]*p + u_m_t*self.pts[1]*p + t*self.pts[2]*p
...
for t in ts:
    ...
    if last_t > 0:
        ...
        print(f"{t:14.5} {p_t:14.5} {dp_dt:14.5} {dsq_t:14.5} {e_dsq_t:14.5}~=0 {p_t_m_p_times_dp_dt:14.5}~={d_dt_dsq_t:14.5} {e_dp_dt:14.5}~=0 {e_d_dt_dsq:14.5}~=0 {e_dt:14.5}~=0")
        pass
    last_t = t
    last_p_t = p_t
    last_dsq_t = dsq_t
