...
enum Table {
    StaticF64(&'static [&'static [f64]]),
    FVec(TypeId, Vec<Vec<u8>>),
}
static CONSTANTS_TABLE_HASHMAP: OnceLock<RwLock<HashMap<(usize, TypeId), Table>>> = OnceLock::new();

fn create_f_table<F: Num>(table: &'static [&'static [f64]]) -> Table {
    let x: Vec<Vec<F>> = table
        .iter()
        .map(|t| {
            t.iter()
                .map(|f| {
    ...
    Table::FVec(TypeId::of::<F>(), unsafe {
        std::mem::transmute::<Vec<Vec<F>>, Vec<Vec<u8>>>(x)
    })
}
fn invoke_f<F: Num, R, FN: FnMut(&[F]) -> R>(mut f: FN, table: &Table, index: usize) -> R {
    match table {
        Table::StaticF64(table) => {
            assert!(TypeId::of::<F>() == TypeId::of::<f64>());
            f(unsafe { std::mem::transmute::<&[f64], &[F]>(&table[index]) })
        }
        Table::FVec(t, table) => {
            assert!(TypeId::of::<F>() == *t);
            return f(unsafe { std::mem::transmute::<&[u8], &[F]>(&table[index]) });
        }
    }
}

fn add_constants_table<F: Num>(table: &'static [&'static [f64]]) {
    let table_address: *const [&'static [f64]] = table;
...
pub fn use_constants_table<F: Num, R, FN: FnMut(&[F]) -> R>(
    f: FN,
    table: &'static [&'static [f64]],
    index: usize,
) -> R {
    let f_type = TypeId::of::<F>();
    let f64_type = TypeId::of::<f64>();
    if f_type == f64_type {
        invoke_f(f, &Table::StaticF64(table), index)
    } else {
        let table_address: *const [&'static [f64]] = table;
        let table_address = table_address.addr();
        let key = &(table_address, f_type);
        ...
        if let Some(table) = map_read.get(key) {
            return invoke_f(f, table, index);
        }
        drop(map_read);
        add_constants_table::<F>(table);
        use_constants_table(f, table, index)
    }
}
