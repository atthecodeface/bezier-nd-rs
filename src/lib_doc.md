...
Elevating a line `(p0, p1)`to a quadratic generates a central control point of `(p0+p1)/2`; elevating a quadatic to a cubic generates two the
two inner control points of `(p0+2c)/3` and `(2c+p1)/3`.
